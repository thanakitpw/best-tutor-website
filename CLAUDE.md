...
## URL Structure (Option B — URL ย่อ + 301 redirect จากเว็บเดิม)

> ตัดสินใจ 2026-04-20: ใช้ **Option B** — URL ย่อให้สะอาด + ทำ 301 redirect จาก URL เว็บเดิม
> อ้าง `docs/seo-migration-audit.md` สำหรับ 301 map

### URL pattern ของเว็บใหม่

| หน้า | URL ใหม่ | 301 จาก (เว็บเดิม) |
|---|---|---|
| หน้าแรก | `/` | — |
| รายวิชาทั้งหมด | `/tutors` | — |
| หมวดวิชาหลัก | `/subject/[category]` | — |
| วิชาย่อย | `/subject/[category]/[sub]` | — |
| โปรไฟล์ติวเตอร์ | `/tutor/[slug]` | `/tutor/[thai-encoded]` → `/tutor/[transliterated-slug]` |
| หาครูสอนพิเศษ | `/find-tutor` | — |
| บทความทั้งหมด | `/blog` | — |
| บทความตามหมวด | `/blog/[category]` | — |
| อ่านบทความ | `/blog/[slug]` | `/{slug}-2026/` (root-level blog posts) |
| สมัครเป็นติวเตอร์ | `/join-with-us` | — |
| ฟอร์มสมัครติวเตอร์ | `/tutor-register` | — |
| หน้ารีวิว | `/review` | — |
| หลังบ้าน | `/admin/*` | — |

### Subject URL mapping (Option B — ย่อจากเว็บเดิม)

| URL ใหม่ | 301 จาก |
|---|---|
| `/subject/english` | `/subject/foreign-language/english/` |
| `/subject/chinese` | `/subject/foreign-language/chinese/` |
| `/subject/japanese` | `/subject/foreign-language/japanese/` |
| `/subject/korean` | `/subject/foreign-language/korean/` |
| `/subject/math` | `/subject/mathematic/` |
| `/subject/science` | `/subject/scient/` |
| `/subject/physics` | `/subject/scient/physics/` |
| `/subject/chemistry` | `/subject/scient/chemistry/` |
| `/subject/biology` | `/subject/scient/biology/` |
| `/subject/social` | `/subject/social-study/` |
| `/subject/thai` | `/subject/thai/` (ไม่เปลี่ยน) |
| `/subject/computer` | `/subject/computer/` (ไม่เปลี่ยน) |
| `/subject/art` | `/subject/art/` (ไม่เปลี่ยน) |
| `/subject/music` | `/subject/music/` (ไม่เปลี่ยน) |
| `/subject/sport` | `/subject/sport/` (ไม่เปลี่ยน) |

**เหตุผล Option B:** เว็บเดิมมี SEO authority น้อย (no JSON-LD, no meta desc) → ไม่เสียหายมากที่เปลี่ยน URL + URL ย่อ share/อ่านง่ายกว่า + 301 redirect แค่ ~25 URLs
...
### Design Fidelity Rule (กฎการทำ UI ตามดีไซน์)

- **Follow Paper design เป็น baseline** — ทุก teammate ต้องดู design ใน Paper (file: "Best Tutor") ก่อนเขียนโค้ด
- **อนุญาตให้ "ปรับให้สวยกว่า" ได้** ถ้ามีเหตุผลชัดเจน เช่น:
  - Conversion / UX ดีขึ้น (เช่น เพิ่ม sticky CTA, ปรับ tutor card มี price+rating ตาม UX analysis)
  - Accessibility ดีขึ้น (เพิ่ม contrast, touch target ขนาดถูก)
  - Mobile UX ดีขึ้น
  - สอดคล้อง Design System ดีขึ้น (ไม่เล่นนอก palette)
- **ห้ามเปลี่ยน:** Layout ใหญ่ ๆ, brand color, typography scale, components structure โดยไม่ปรึกษา Lead
- **ก่อนปรับต้อง:** เขียน comment สั้น ๆ ว่าปรับอะไร + เหตุผล (ไม่ต้อง approve ล่วงหน้า ถ้าชัดเจน)
- **ดู design:** Paper MCP — tools `get_screenshot`, `get_jsx`, `get_computed_styles` สำหรับ exact values
- **อ้าง `docs/ux-ui-analysis.md`** สำหรับสิ่งที่ design ยังขาด (sticky CTA, tutor card fields, filter sidebar, etc.) — สิ่งเหล่านี้ **ต้องเพิ่ม** เพราะช่วย Lead/SEO
...
- Phase 1 Design 95% done — Phase 2 เริ่มแล้ว 2026-04-20
- ลูกค้ามี data ครบ (WordPress export, รายชื่อติวเตอร์ 103 คน, รูป 65 คน, บทความ 12 ตัว)
- **Build-first strategy:** สร้างเว็บให้เสร็จก่อน โดยใช้ **seed/mock data** — feed ข้อมูลจริง (ติวเตอร์ + บทความ) เข้าผ่าน Admin CMS ใน Phase 8 หลังระบบพร้อม
- URL strategy: **Option B** (URL ย่อ + 301 redirect) — ดู `docs/seo-migration-audit.md`
- รูปภาพจากเว็บเดิมใช้ได้ แต่ Logo ควรขอต้นฉบับ
- **การทำงาน:** ใช้ Agent Team ตาม section "Agent Team Workflow" ด้านบน — Lead (session นี้) ไม่เขียน feature code เอง
- **การทำ UI:** ตาม Design Fidelity Rule ด้านบน — follow Paper designs เป็น baseline, ปรับให้สวย/ใช้งานดีกว่าได้ถ้ามีเหตุผล
